var slide=function(){var slideWrap=$(".slide"),listButtonBox=$('.buttonlist'),roll=slideWrap.children(),list=roll.children(),len=list.length,move=list.height(),fx='easeInSine',curIndex=0,timer=null,speed=200,auto=5000;var init=function(){list.eq(0).css('zIndex',2).siblings().hide();var s="";for(var i=0;i<len;i++){if(i===curIndex){s+='<a  class="active" href="javascript:;"></a>';}else{s+='<a href="javascript:;"></a>';}}
...
init();}
function bannerSlide(){var box=$('.ag7-bannerslide .slidebox'),buttons=$('.ag7-bannerslide .slidelist'),mask=$('.ag7-bannerslide .mask'),h1=$('.ag7-bannerslide h2'),h2=$('.ag7-bannerslide h3'),list=box.children('li'),len=list.length,curIndex=0,nextIndex=1,speed=200,timer=null,delay=null,duration=5000;var setTitle=function(index){var li=buttons.find('li').eq(index);h1.html(li.data('h1'));h2.html(li.data('h2'));}
...
mask.stop(true,true).animate({top:buttons.find('li').eq(next).position().top+0},300,'easeOutExpo');setTitle(next);curIndex=next;}
